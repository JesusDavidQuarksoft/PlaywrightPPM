...
export class ProjectsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.sectorCheckbox = page.getByRole("checkbox", {
      name: "Sector",
      exact: true,
    });
    this.waterAndEnvironmentCheckbox = page.getByRole("checkbox", {
      name: "Agua y Medio Ambiente",
    });
    this.cellColumnWaterAndEnvironment = page
      .getByRole("cell", { name: "Agua y Medio Ambiente" })
      .first();
    this.verifyWaterAndEnvironmentTable = page.locator("canvas");
    this.transportCheckbox = page.getByRole("checkbox", { name: "Transporte" });
    this.cellColumnTransport = page
      .getByRole("cell", { name: "Transporte" })
      .first();
    this.btnExportTablePDF = page
      .locator("app-file-export-component > button")
      .first();
  ...
  async filterByHidrocarbons(): Promise<void> {
    await this.filterHidrocarbonsTable.waitFor({
      state: "visible",
      timeout: 10000,
    });
  ...
  async verifyWaterAndEnvironmentTableVisible(): Promise<void> {
    await this.verifyWaterAndEnvironmentTable.waitFor({
      state: "visible",
      timeout: 10000,
    });

    await this.verifyWaterAndEnvironmentTable.click({
      position: {
        x: 383,
        y: 190,
      },
